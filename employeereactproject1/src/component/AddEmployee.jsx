...
const EmployeeRegistrationForm = () => {
  const [employee, setEmployee] = useState({
    name: '', email: '', phone: '', position: '', basicPay: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEmployee({ name: '', email: '', phone: '', position: '', basicPay: '' });
    } catch (err) {
      setError(true);
      ...
      <Container maxWidth="sm">
        <Box sx={{ mt: 4, p: 4, boxShadow: 3, borderRadius: 2 }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Button variant="contained" type="submit" fullWidth sx={{ mt: 2, backgroundColor: 'black', color: 'white', '&:hover': { backgroundColor: '#333' } }}>
              Register
            </Button>
          </form>
